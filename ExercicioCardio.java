public class ExercicioCardio extends Exercicio {
    ...
    public ExercicioCardio(String nome, int tempo, double metaDistancia) {
        super(nome, "Cardio", 0, 1, 1, 0);
        if (tempo <= 0) throw new AppException("O tempo do exercício cardio deve ser maior que zero.");
        if (metaDistancia < 0) throw new AppException("A meta de distância não pode ser negativa.");
        this.tempo = tempo;
